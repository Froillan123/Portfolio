...
export const ecosystemOverview = {
  ...
  description:
    "FaceOfMind is an end-to-end platform engineering project built on Google Cloud, decomposing a multi-service clinical application into independently managed infrastructure tiers: a custom Go Radix Trie API gateway, a declarative GitOps matrix control plane, zero-disk KMS envelope encryption vaults, and private Cloud Run workloads.",
...
export const projects: Project[] = [
  {
    ...
    bullets: [
      ...
      "Automated least-privilege IAM service account bindings and transactional route synchronization with PostgreSQL append-only audit triggers and LISTEN/NOTIFY events",
    ],
    ...
    securityGuarantees: [
      ...
      "Append-only PostgreSQL audit vault with database triggers that reject all UPDATE, DELETE, and TRUNCATE attempts",
  ...
  },
  {
    ...
    bullets: [
      "Lock-free Radix Trie router delivering deterministic O(k) path matching with low-overhead in-memory routing",
      "3-State in-memory Circuit Breaker (CLOSED, HALF-OPEN, OPEN) with exponential backoff, designed to prevent cascading failures across downstream services",
      "Obtains Google-signed OIDC identity tokens from the Compute/Cloud Run metadata server and presents them when invoking IAM-protected private Cloud Run services",
    ...
    problemSolved: {
      ...
      impact: "Delivered low-overhead O(k) path matching, zero-redeploy route sync, and robust failure isolation on serverless compute.",
    },
  },
  {
    id: "kms-secret-vault",
    ...
    bullets: [
      ...
      "Disabled Linux core dumps with RLIMIT_CORE=0 to reduce the risk of sensitive key material being persisted in crash dumps",
      "Implemented POSIX mlockall() memory page locking to prevent resident secrets in RAM from being paged to swap space",
      "Built an append-only PostgreSQL audit vault protected by database triggers that reject all UPDATE, DELETE, and TRUNCATE attempts",
    ],
    ...
    securityGuarantees: [
      ...
      "Crash dump suppression (RLIMIT_CORE=0) prevents process memory from persisting in core dump files",
      "POSIX memory page locking (mlockall) prevents resident RAM keys from being paged to swap",
      "Database trigger trg_prevent_audit_log_tampering enforces strict append-only audit trail by rejecting mutation commands",
    ],
    problemSolved: {
