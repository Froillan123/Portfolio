...
export function MermaidViewer({ chart, className = "", title = "Architecture Diagram" }: MermaidViewerProps) {
  ...
  const handleFullZoomIn = () => setFullZoom((prev) => Math.min(prev + 0.15, 3.5));
  const handleFullZoomOut = () => setFullZoom((prev) => Math.max(prev - 0.15, 0.3));
  ...
  const handleWheel = (e: React.WheelEvent) => {
    if (e.ctrlKey || e.metaKey || e.altKey) {
      ...
      setZoom((prev) => Math.min(Math.max(prev + delta, 0.35), 3.0));
    }
  };
  ...
  const handleFullWheel = (e: React.WheelEvent) => {
    if (e.ctrlKey || e.metaKey || e.altKey) {
      e.preventDefault();
      const delta = e.deltaY < 0 ? 0.1 : -0.1;
      setFullZoom((prev) => Math.min(Math.max(prev + delta, 0.3), 3.5));
    }
  };
    ...
    <>
      ...
      >
        ...
        <div className="flex items-center justify-between gap-1.5 border-b border-border/60 bg-muted/30 px-2 py-1.5 sm:px-4 sm:py-2 text-xs w-full">
          ...
          <div className="flex items-center gap-1 shrink-0">
            ...
            <Button
              size="sm"
              variant="ghost"
              className="h-6.5 sm:h-7 px-1.5 sm:px-2 text-[10px] sm:text-xs font-mono text-muted-foreground hover:text-foreground hover:bg-muted rounded-md border border-border/50 gap-1 shrink-0"
              onClick={() => {
                setFullZoom(1);
                setFullPan({ x: 0, y: 0 });
                setIsExpanded(true);
            ...
            <Button
              size="sm"
              variant="ghost"
              className="h-6.5 sm:h-7 px-1.5 sm:px-2 text-[10px] sm:text-xs font-mono text-muted-foreground hover:text-foreground hover:bg-muted rounded-md border border-border/50 shrink-0"
        ...
        >
          ...
          {!loading && !hasError && (
            <div className="absolute bottom-1.5 right-1.5 sm:bottom-3 sm:right-3 z-10 grid grid-cols-3 grid-rows-3 gap-0.5 sm:gap-1 rounded-lg sm:rounded-xl border border-border/80 bg-background/95 p-1 shadow-lg backdrop-blur-md scale-90 sm:scale-100 origin-bottom-right">
              {/* Row 1 */}
              <div className="size-6 sm:size-7" />
              <Button
                variant="ghost"
                size="icon"
                className="size-6 sm:size-7 text-muted-foreground hover:text-foreground hover:bg-muted rounded border border-border/40"
                onClick={() => handlePan(0, 45)}
                title="Pan up"
                aria-label="Pan up"
              >
        ...
        createPortal(
          ...
          >
            ...
            <div className="flex items-center justify-between gap-3 border-b border-border/80 bg-card/95 px-3 py-2.5 sm:px-6 sm:py-3.5 text-xs shrink-0 shadow-sm">
              ...
              <div className="flex items-center gap-2 shrink-0">
                <Button
                  size="sm"
                  variant="outline"
                  className="h-8 px-2.5 text-xs font-mono rounded-lg border-border/70"
                  onClick={copyDefinition}
                >
                  {copied ? (
                ...
                <Button
                  size="sm"
                  variant="secondary"
                  className="h-8 px-3 text-xs font-medium rounded-lg bg-muted hover:bg-muted/80 gap-1.5"
            ...
            <div
              ...
              className={`relative flex-1 w-full h-full overflow-hidden p-4 sm:p-8 flex items-center justify-center bg-muted/15 dark:bg-zinc-950 ${
            ...
            >
              ...
              <div className="absolute bottom-4 right-4 sm:bottom-6 sm:right-6 z-10 grid grid-cols-3 grid-rows-3 gap-1 rounded-xl border border-border/80 bg-background/95 p-1.5 shadow-2xl backdrop-blur-md">
                {/* Row 1 */}
                <div className="size-7 sm:size-8" />
                <Button
                  variant="ghost"
                  size="icon"
                  className="size-7 sm:size-8 text-muted-foreground hover:text-foreground hover:bg-muted rounded-md border border-border/50"
                  onClick={() => handleFullPan(0, 60)}
                  title="Pan up"
                  aria-label="Pan up"
                >
